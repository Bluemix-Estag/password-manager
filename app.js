        ...
        if (err) {
            ...
            for (var user in doc['users']) {
                if ((doc['users'][user]['username']) == logInfo.login && doc['users'][user]['password'] == logInfo.password) {
                    ...
                    var accounts = [];
                    // for each because when it deletes it stops being sequential
                    for (var account in doc['users'][user]['accounts']) {
                        accounts.push(doc['users'][user]['accounts'][account]);
                    }
                    res.render('profile', {
                        contas: accounts
                    });
...
// Insert account with deleted id.
        console.log(doc['users'][AUTHENTICATED_USER]['accounts']);
        db.insert(doc, DOCUMENT_ID, function (err, doc) {
            if (err) {
                console.log(err);
            } else {
